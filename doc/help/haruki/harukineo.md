...
本部分的全部教程文档由 [HarukiBot开发者们](#关于) 编写，感谢他们的付出。
:::
...
- 如果你在使用过程中遇到任何问题，你可以在该页面最下方的 [关于](#关于) 下面联系 [开发者](#关于) 进行反馈。
...
- HarukiBot NEO支持日服（jp）、台服（tw）、韩服（kr）、国际服（en）以及国服（cn） 如果需要使用对应区服的功能需要在指令前加区服前缀（如`/cn个人信息`）
- HarukiBot NEO现在支持**全局默认绑定账号**和**区服默认绑定账号**，区服相关指令(如sk、sk线、时速等)如果不带区服前缀，会默认采用**您的全局默认绑定账号**的区服
...
> 台服/韩服 Android用户教程参考[Haruki工具箱-HarukiProxy使用教程](https://neo.haruki.seiunx.com/harukiproxy/)
> iOS / iPadOS 用户 建议使用代理工具MitM模块更新，教程参考[Haruki工具箱-iOS模块上传数据教程](https://neo.haruki.seiunx.com/toolboxtutorial/ios-module.html)

> ⚠️ **所有 MySekai 指令需用户绑定 Haruki工具箱 账号** ⚠️ **本功能不支持国服**
...
> 若违反相关条例将会视情况采取删除对应昵称至禁止使用Bot不等的措施。 审核别名需要对应权限，为管理员人工进行，请不要添加网络垃圾增加大家的工作量
...
- 联系我：[admin@shiromiku.moe](mailto:admin@shiromiku.moe)或QQ：`57892198`
